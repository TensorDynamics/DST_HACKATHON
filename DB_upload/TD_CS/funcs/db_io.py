...
def get_uq_vals_from_table(sql_engine, schema, table_name, col_name, where_statement_dict=None, **kwargs):
    """
...
def get_site_init_dates_map(con, table_name, schema):
    """
    ...
    :return: unique of site_name, timestamp and init_date
    """
    ...
    for name in ['site_name', 'init_date', 'times']:
        if name not in col_names_of_table:
            raise KeyError(f"Could not find {name} in {schema}.{table_name} ")

    # READ DATA AND RETURN DICT
    db_site_file_map = read_sql_query(sql=f"select site_name, init_date "
                                          f"from {schema}.{table_name} "
                                          f"group by site_name, init_date",
                                      con=con)

    db_site_file_map = db_site_file_map.sort_values(by=['site_name', 'init_date'],
                                                    ascending=True).reset_index(drop=True)

    return db_site_file_map


def delete_records_from_table_site_date(con, site_name, date, schema, table_str):
    """
    Deletes records from an existing table based on a site and date 'AND' combination
    :param con: db connection
    ...
    :param table_str: table name
    """
    delete_query = f"DELETE FROM {schema}.{table_str} " \
                   f"WHERE times::date = '{date}' AND site_name = '{site_name}'"
    con.execute(delete_query)


def upsert_dataframe_to_db(data_frame,
                           db_con,
                           schema,
                           table_name,
                           logger=None):
    """
    Upsert dataframe on site name and date
    :param data_frame: data frame
    :param logger: logger object, optional, defaults to no logging
    :param db_con: db connection
    :param schema: schema
    :param table_name: table to perform upsert on
    """
    if data_frame.shape[0] > 0:
        date_site_identifiers = data_frame[['site_name', 'times']].copy()
        date_site_identifiers['date'] = date_site_identifiers['times'].dt.date

        for site in sorted(date_site_identifiers['site_name'].unique()):
            append_one_go_site = pd.DataFrame()
            for date in sorted(date_site_identifiers['date'].unique()):
                # DELETE
                delete_records_from_table_site_date(con=db_con,
                                                    schema=schema,
                                                    table_str=table_name,
                                                    site_name=site,
                                                    date=date)

                # APPEND TO A DF FOR APPENDING
                append_data = data_frame[(data_frame['times'].dt.date == date) &
                                         (data_frame['site_name'] == site)].copy()
                append_one_go_site = pd.concat([append_one_go_site, append_data], axis=0)

            # APPEND IN ONE GO FOR SITE - BETTER TIMING
            append_data_to_table(data=append_one_go_site,
                                 ...
                                 logger_obj=logger)
            if logger:
                logger.info(f"Upsert successful : {site} : {sorted(append_one_go_site['times'].dt.date.unique())}")
...
def get_db_table_info(db_connection, table_name, schema_name):
    """ Extracts the following
    1. files available in the table by identifying 'file' column
    2. site to file maps ({site: [file1, file2, ...], ...})
    3. Table column names as list
    """
    files_in_db = get_uq_vals_from_table(sql_engine=db_connection,
                                         schema=schema_name,
                                         table_name=table_name,
                                         col_name='init_date')

    db_site_file_map = get_site_init_dates_map(con=db_connection,
                                               table_name=table_name,
                                               schema=schema_name)

    table_columns = get_table_col_names(con=db_connection,
                                        table_str=table_name,
                                        schema=schema_name)
    return files_in_db, db_site_file_map, table_columns


def infer_files_to_read(input_locations, local_files, db_site_file_map) -> list:
    """ Logic to identify new files and new locations and which files to read"""
    locations_in_db = list(db_site_file_map.keys())
    files_in_db = list(set([item for sublist in db_site_file_map.values() for item in sublist]))
    ...
    if new_locations:
        ...
        files_missing_for_any_site = [file for file in files_missing_for_any_site if file in local_files]
        files_to_read = files_missing_for_any_site + new_files
        return sorted(files_to_read)


def get_site_forecast_status(db_connection, table, schema, site_col, date_col):
    query = f"select site_name, snapshot_date from {schema}.{table} GROUP BY 1, 2"
    data = pd.read_sql_query(sql=query, con=db_connection)
    return {site: list(data[data[site_col] == site][date_col].unique()) for site in data[site_col].unique()}
